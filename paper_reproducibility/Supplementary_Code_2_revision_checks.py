#!/usr/bin/env python3
"""Revision-stage robustness checks for the Sn-Ge perovskite screening study."""
...
def grid_model(EgI0=1.31, EgBr0=1.77, b=0.13, EgI_z05=1.50, EgGeBr=2.50):
    xs=np.round(np.arange(0,1.0001,0.05),2); zs=np.round(np.arange(0,0.5001,0.05),2)
    alphaI=(EgI_z05-EgI0)/0.50; alphaBr=EgGeBr-EgBr0; rows=[]
    for z in zs:
        for x in xs:
            EgI=EgI0+alphaI*z; EgBr=EgBr0+alphaBr*z
            Eg=(1-x)*EgI+x*EgBr-b*x*(1-x)
            Eh=0.0118-5.00e-4*x-1.17778e-2*z+5.5556e-4*x*z
            Eox=-1.374-0.0550*x+1.37333*z+0.055556*x*z
            rB=(1-z)*1.18+z*0.73; rX=(1-x)*2.20+x*1.96
...
def score_arrays(df,Egstar,sig,tau,lam,t0,beta,weights):
    Eg=df.Eg_eV.to_numpy(); Eh=df.Ehull_proxy_eV_atom.to_numpy()
    Eox=df.Eox_proxy_eV_Sn.to_numpy(); t=df.t.to_numpy()
    Sg=np.exp(-((Eg-Egstar)**2)/(2*sig**2)); Sh=np.exp(-np.maximum(Eh,0)/tau)
    Sox=np.exp((Eox-Eox.max())/lam); St=np.exp(-beta*(t-t0)**2)
    C=np.vstack([Sg,Sh,Sox,St]); w=np.asarray(weights)[:,None]
    return np.exp((w*np.log(C)).sum(axis=0)/w.sum())

def pareto_mask(obj):
    n=len(obj); out=np.ones(n,dtype=bool)
    for i in range(n):
        dom=np.all(obj<=obj[i],axis=1)&np.any(obj<obj[i],axis=1); dom[i]=False
        if dom.any(): out[i]=False
    return out

def lhs_metrics(df,n,seed=20260825,app="single"):
    sampler=qmc.LatinHypercube(d=10,seed=seed); U=sampler.random(n)
    ...
    P=lo+U*(hi-lo); top10=np.zeros(len(df),int); top1=np.zeros(len(df),int); ranksum=np.zeros(len(df),float)
    for p in P:
        scores=score_arrays(df,*p[:6],p[6:]); order=np.argsort(-scores,kind="mergesort")
...
def main():
    plt.rcParams.update({"font.size":10,"axes.titlesize":11,"axes.labelsize":11,"xtick.labelsize":9,"ytick.labelsize":9,"legend.fontsize":9})
    df=grid_model()
    ...
    df.to_csv(DATA_DIR/"Supplementary_Data_S4_Pareto_Grid.csv",index=False)

    rows=[]
    for n in [500,1000,2500,5000,10000]:
        t10,t1,mr=lhs_metrics(df,n)
        for z in [0.40,0.45,0.50]:
            idx=df.index[(df.x_Br==0)&(df.z_Ge==z)][0]; rows.append((n,z,t10[idx],t1[idx],mr[idx]))
    conv=pd.DataFrame(rows,columns=["draws","z_Ge","top10_fraction","top1_fraction","mean_rank"])
    conv.to_csv(DATA_DIR/"Supplementary_Data_S5_LHS_Convergence.csv",index=False)

    pred_sn=1.77; pred_eq=1.77+(2.50-1.77)*0.5; pred_ge=2.50
    ...
    challenge.to_csv(DATA_DIR/"Supplementary_Data_S6_Hooper_Optical_Challenge.csv",index=False)

    fig,ax=plt.subplots(figsize=(6.8,4.8)); y=eoxmax-df.Eox_proxy_eV_Sn
    ax.scatter(np.abs(df.Eg_eV-1.34),y,s=18,alpha=0.45,label="All 231 compositions")
    pm=df.pareto_single.to_numpy()
    ax.scatter(np.abs(df.Eg_eV[pm]-1.34),y[pm],s=32,label="Four-objective Pareto set")
    ax.set_xlabel(r"Optical mismatch, $|E_g-1.34|$ (eV)")
    ax.set_ylabel(r"Oxidation penalty, $\Delta E_{ox,max}^{proxy}-\Delta E_{ox}^{proxy}$ (eV/Sn)")
    ax.legend(frameon=False); ax.set_title("Weight-free Pareto diagnostic: single-junction target")
    save_both(fig,"Figure_5_Pareto_Diagnostic")

    fig,ax=plt.subplots(figsize=(6.8,4.8))
    for z,g in conv.groupby("z_Ge"): ax.plot(g.draws,100*g.top10_fraction,marker="o",label=f"z = {z:.2f}, x = 0")
    ax.set_xlabel("Latin-hypercube draws"); ax.set_ylabel("Top-decile occupancy (%)")
    ax.set_title("Convergence of the single-junction robustness metric"); ax.legend(frameon=False)
    save_both(fig,"Figure_4_LHS_Convergence")

    fig,ax=plt.subplots(figsize=(6.8,4.8)); zz=np.linspace(0,1,101); pred=1.77+(2.50-1.77)*zz
    ...
    ax.axvline(0.5,linestyle="--",linewidth=1); ax.set_xlabel("Ge fraction, z (bromide series)")
    ax.set_ylabel(r"Optical band gap, $E_g$ (eV)"); ax.set_title("Independent synthesis-resolved optical challenge")
    ax.legend(frameon=False,fontsize=8); save_both(fig,"Figure_6_External_Optical_Challenge")
    ...
    ax.text(0.89,0.80,"Validation windows",ha="center",fontsize=12)
