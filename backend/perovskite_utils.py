...
rA, rB, rX = IONIC_RADII["Cs"], IONIC_RADII["Sn"], IONIC_RADII[hal]
...
Eg = (1-z)Eg_Sn + zEg_Ge
Eh = (1-z)Eh_Sn + zEh_Ge
dEox = (1-z)dEox_Sn + zdEox_Ge
sbg = _score_band_gap(Eg, lo, hi, center, sigma)
raw = sbg * math.exp(-Eh/(alpha0.0259)) * math.exp(dEox/K_T_EFF) * math.exp(-betaabs((rA+rX)/(math.sqrt(2)(rB+rX))-0.95))
pce = sq_efficiency(Eg)
rows.append({
"x": round(x,3), "z": round(z,2), "Eg": round(Eg,3), "Ehull": round(Eh,4), "Eox": round(dEox,3),
"formula": f"{A}-{B} x={x:.2f} z={z:.2f}",
"PCE_max (%)": round(pce100,1),
"raw": raw
})
if not rows:
return pd.DataFrame()
...
rows.append({
"x": round(x,3), "y": round(y,3), "z": round(z,2), "Eg": round(Eg,3), "Ehull": round(Eh,4), "Eox": round(dEox,3),
