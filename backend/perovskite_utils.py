...
def oxidation_energy(formula_sn2: str) -> float:
    """ΔEₒₓ per Sn for CsSnX₃ + ½ O₂ → ½(Cs₂SnX₆ + SnO₂). Positive ⇒ uphill."""
...
def screen_binary(
    ...
    *, z: float = 0.0, application: str|None = None
    ...
    """
    Screen A–B mix; `application` chooses Gaussian band-gap weighting.
    """
    lo, hi = bg
    center=sigma=None
...
def mix_abx3(
    ...
    *, z: float=0.0, alpha: float=1.0, beta: float=1.0,
    ...
    for x in np.arange(0.0,1.0+1e-9,dx):
        ...
        rows.append({"x":round(x,3),"z":round(z,2),
...
def screen_ternary(
    ...
    *, dx: float=0.10, dy: float=0.10, z: float=0.0,
    ...
    for r in rows:
        r["score"]=round(r.pop("raw")/raw_max,3)
    return(
        pd.DataFrame(rows)
        .sort_values("score",ascending=False)
        .reset_index(drop=True)
