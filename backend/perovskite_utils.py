...
# ── Default environmental‐penalty calibration ──────────────────────────
...
def mix_abx3(
    ...
    bowing: float = 0.0,
    dx: float = 0.05,
    alpha: float = ALPHA_RH,   # now accepts slider override
    beta: float  = BETA_T      # now accepts slider override
) -> pd.DataFrame:
    ...
    for x in np.arange(0, 1 + 1e-6, dx):
        for y in np.arange(0, 1 - x + 1e-6, dy):
            ...
            rows.append({
                ...
                "Eg": round(Eg,3), "score": round(score,3)
            })
    return pd.DataFrame(rows).sort_values("score", ascending=False).reset_index(drop=True)
