...
ADDITIVE_PENALTIES = {
    "none":  0.0,
    "SnF2":  +0.50,   # example binding energy (eV)
    "NH4SCN":+0.35,
    "PEABr": +0.40,
}
...
# ─────────── binary screen ───────────
def screen_binary(A:str,B:str,rh:float,temp:float,bg:tuple[float,float],bow:float,dx:float,*,
                  z:float=0.0,application:str|None=None, additive:str="none") -> pd.DataFrame:
    ...
    return mix_abx3(A,B,rh,temp,(lo,hi),bow,dx,z=z,center=center,sigma=sigma, additive=additive)

def mix_abx3(A:str,B:str,rh:float,temp:float,bg:tuple[float,float],bow:float,dx:float,*,
             z:float=0.0,alpha:float=1.0,beta:float=1.0,center:float|None=None, sigma:float|None=None,
             additive:str="none") -> pd.DataFrame:
    lo,hi = bg
