backend/perovskite_utils.py  (v9.6, fixed exports)
...
from backend.sq import sq_efficiency  # pce helper
...
if "Sn" not in formula_sn2: return 0.0
hal = next((h for h in ("I","Br","Cl") if h in formula_sn2), None)
if hal is None: return 0.0
def formation_energy_fu(f: str) -> float:
doc = fetch_mp_data(f, ["formation_energy_per_atom"])
if not doc or doc.get("formation_energy_per_atom") is None:
raise ValueError(f"Missing formation-energy for {f}")
comp = Composition(f)
...
def screen_binary(A: str, B: str, rh: float, temp: float, bg: tuple[float,float], bow: float, dx: float, *, z: float=0.0, application: str|None=None) -> pd.DataFrame:
...
return mix_abx3(A,B,rh,temp,(lo,hi),bow,dx,z=z,center=center,sigma=sigma)

def mix_abx3(A: str, B: str, rh: float, temp: float, bg: tuple[float,float], bow: float, dx: float, , z: float=0.0, alpha: float=1.0, beta: float=1.0, center: float|None=None, sigma: float|None=None) -> pd.DataFrame:
lo,hi = bg
...
# Sn/Ge mix
Eg_Sn=(1-x)dA["band_gap"]+xdB["band_gap"]-bowx*(1-x)
Eh_Sn=(1-x)dA["energy_above_hull"]+xdB["energy_above_hull"]
dEox_Sn=(1-x)oxA+xoxB
Eg_Ge=(1-x)dA_Ge["band_gap"]+xdB_Ge["band_gap"]-bowx(1-x)
Eh_Ge=(1-x)dA_Ge["energy_above_hull"]+xdB_Ge["energy_above_hull"]
dEox_Ge=(1-x)oxA_Ge+xoxB_Ge
Eg=(1-z)Eg_Sn+zEg_Ge; Eh=(1-z)Eh_Sn+zEh_Ge; dEox=(1-z)dEox_Sn+zdEox_Ge
sbg=_score_band_gap(Eg,lo,hi,center,sigma)
raw=sbgmath.exp(-Eh/(alpha0.0259))math.exp(dEox/K_T_EFF)math.exp(-betaabs((rA+rX)/(math.sqrt(2)(rB+rX))-0.95))
pce= sq_efficiency(Eg)
rows.append({"x":round(x,3),"z":round(z,2),"Eg":round(Eg,3),"Ehull":round(Eh,4),"Eox":round(dEox,3),"formula":f"{A}-{B} x={x:.2f} z={z:.2f}","PCE_max (%)":round(pce*100,1),"raw":raw})
...
def screen_ternary(A: str, B: str, C: str, rh: float, temp: float, bg: tuple[float,float], bows: dict[str,float], , dx: float=0.10, dy: float=0.10, z: float=0.0, application: str|None=None) -> pd.DataFrame:
lo,hi=bg; center=sigma=None
if application in APPLICATION_CONFIG:
cfg=APPLICATION_CONFIG[application]
lo,hi=cfg["range"]; center,sigma=cfg["center"],cfg["sigma"]
dA=fetch_mp_data(A,["band_gap","energy_above_hull"]); dB=fetch_mp_data(B,["band_gap","energy_above_hull"]); dC=fetch_mp_data(C,["band_gap","energy_above_hull"])
...
w=1.0-x-y
Eg_Sn=wdA["band_gap"]+xdB["band_gap"]+ydC["band_gap"]-bows["AB"]xw-bows["AC"]yw-bows["BC"]xy
Eg_Ge=wdA_Ge["band_gap"]+xdB_Ge["band_gap"]+ydC_Ge["band_gap"]-bows["AB"]xw-bows["AC"]yw-bows["BC"]xy
Eg=(1-z)Eg_Sn+zEg_Ge
Eh_Sn=wdA["energy_above_hull"]+xdB["energy_above_hull"]+ydC["energy_above_hull"]
Eh_Ge=wdA_Ge["energy_above_hull"]+xdB_Ge["energy_above_hull"]+ydC_Ge["energy_above_hull"]
Eh=(1-z)Eh_Sn+zEh_Ge
dEox=woxA+xoxB+yoxC
sbg=_score_band_gap(Eg,lo,hi,center,sigma)
raw=sbg*math.exp(-Eh/0.0518)*math.exp(dEox/K_T_EFF)
rows.append({"x":round(x,3),"y":round(y,3),"z":round(z,2),"Eg":round(Eg,3),"Ehull":round(Eh,4),"Eox":round(dEox,3),"formula":f"{A}-{B}-{C} x={x:.2f} y={y:.2f} z={z:.2f}","raw":raw})
if not rows: return pd.DataFrame()
m=max(r["raw"] for r in rows) or 1.0
for r in rows: r["score"]=round(r.pop("raw")/m,3)
return pd.DataFrame(rows).sort_values("score",ascending=False).reset_index(drop=True)
