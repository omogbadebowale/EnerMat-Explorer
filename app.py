...
with tab_plot:
    if mode == "Binary A–B":
        ...
        fig = px.scatter(
            ...
            y="Eg",
            color="score",
            color_continuous_scale="Turbo",  # keep same vibrant palette
            hover_data=["formula","x","Eg","stability","score"],
            width=1200, height=800  # publication-res dimensions
        )
        ...
        fig.update_layout(
            ...
            font=dict(family="Times New Roman", size=24, color="#333"),  # larger body text
            xaxis=dict(
                title="Stability",
                title_font_size=26,
                tickfont_size=18,
        ...
        # high-res download (requires kaleido)
        ...
        fig3d = px.scatter_3d(
            ...
            x="x", y="y", z="score",
            color="score",
            color_continuous_scale="Turbo",  # keep vibrant palette
            hover_data={k: True for k in ("x","y","Eg","score") if k in plot_df},
            width=1200, height=900  # publication dimensions
        )
        fig3d.update_traces(marker=dict(size=8, opacity=0.9, line=dict(width=1, color="black")))
        fig3d.update_layout(
            template="plotly_white",
            margin=dict(l=80, r=80, t=60, b=60),
            font=dict(family="Times New Roman", size=24, color="#222"),  # larger base font
            scene=dict(
                aspectmode='cube',
                camera=dict(
...
with tab_dl:
    ...
    for prop, val in rows:
        ...
        cells[0].text, cells[1].text = prop, str(val)
    buf = io.BytesIO()
